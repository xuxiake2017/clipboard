...
- 可通过 `CLIPBOARD_CREATE_ALLOWED_IPS` 限制只有指定客户端 IP 可以新建剪贴板；已存在剪贴板仍可凭密码进入。
...
默认不限制新建剪贴板。如果只允许特定客户端 IP 新建，在 `.env.local` 配置：

```bash
CLIPBOARD_CREATE_ALLOWED_IPS=192.168.1.10,203.0.113.8
```

应用会优先从反向代理的 `X-Real-IP` 获取客户端 IP，其次读取 `X-Forwarded-For`。因此使用 Nginx/Caddy 时请正确传递真实 IP。
